-- Compatibilidad ZUARA para autenticación propia (Neon Function).
create or replace function public.zuara_user_has_permission(p_user_id bigint, p_permiso text)
returns boolean
language plpgsql
stable
as $$
declare raw text;
begin
  select coalesce(permisos,'[]') into raw from public.usuarios where id=p_user_id;
  if raw is null then return false; end if;
  begin
    if left(trim(raw),1)='[' then
      return exists(select 1 from jsonb_array_elements_text(raw::jsonb) x where x=p_permiso)
             or (p_permiso='agregar_tasa' and exists(select 1 from jsonb_array_elements_text(raw::jsonb) x where x='parametros'));
    end if;
    return coalesce((raw::jsonb ->> p_permiso)::boolean,false)
           or (p_permiso='agregar_tasa' and coalesce((raw::jsonb ->> 'parametros')::boolean,false));
...
create or replace function public.zuara_validar_fecha_iso_real(p_fecha text)
returns boolean
language plpgsql
immutable
as $$
begin
  if p_fecha !~ '^\d{4}-\d{2}-\d{2}$' then return false; end if;
...
begin
  ...
  if p_action = 'registrar_movimiento' then
    if not zuara_user_has_permission(u.id,'movimientos') then
  ...
  if p_action = 'registrar_tasa' then
    if not zuara_user_has_permission(u.id,'agregar_tasa') then
      return jsonb_build_object('error','No es posible realizar esta operación.','status_code',403);
    end if;
    fecha_tasa := coalesce(p_payload->>'fecha','');
    if not zuara_validar_fecha_iso_real(fecha_tasa) then
  ...
  if p_action = 'guardar_configuracion' then
    if not zuara_user_has_permission(u.id,'configuracion') then
      return jsonb_build_object('error','No es posible realizar esta operación.','status_code',403);
    end if;
  ...
  if p_action = 'registrar_venta' then
    if not zuara_user_has_permission(u.id,'ventas') then
      return jsonb_build_object('error','No es posible realizar esta operación.','status_code',403);
    end if;
    ...
    if not zuara_validar_fecha_iso_real(fecha_fact) then
  ...
  if p_action = 'registrar_devolucion' then
    if not zuara_user_has_permission(u.id,'ventas') and not zuara_user_has_permission(u.id,'historial_ventas') then
      return jsonb_build_object('error','No es posible realizar esta operación.','status_code',403);
    end if;
